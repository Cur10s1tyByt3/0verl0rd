...
//!     module list and export tables by hand. Module whose names it needs are
//!     matched by a rotate hash of *immediate* constants — never through
//!     `.rodata` string literals, because a RIP-relative load into the
//!     file-offset-mapped raw copy would read the wrong bytes until the image
//!     has been relocated.
...
// rotate-right-by-1 hashes of the names the loader resolves, derived with the
// exact hash below (`kernel32.dll`, `ntdll.dll`, export names). Pure immediates:
// keeping them out of `.rodata` is what keeps the loader relocation-free.
...
/// Equivalent to the C loader's `caller()` walk.
...
/// ror hash of a UTF-16 code-unit buffer (case-normalized as in the C loader:
/// `a`..`z` map to `A`..`Z`, everything else keeps its value).
...
pub extern "system" fn loader_impl(lpParameter: usize) -> usize {
    unsafe {
        ...
        // Register .pdata so unwinding through our hooks does not crash.
        // We already have ntdll's base from the module walk; the proc-name
        // string is materialized from immediates so no .rodata access is made.
        ...
        if reloc_size != 0 {
            ...
            loop {
                ...
                while count > 0 {
                    ...
                    let typ = (word >> 12) & 0xF;
                    let off = word & 0xFFF;
                    // Only DIR64 (10) needs applying; ABSOLUTE (0) is a no-op
                    // and HIGHLOW/HIGH/LOW never occur in an x64 GNU build.
                    // A single comparison is used deliberately: a multi-case
                    // dispatch lets LLVM emit a jump table in `.rodata`, whose
                    // RIP-relative address would be wrong in the raw copy.
                    if typ == 10 {
...
mod tests {
    ...
    #[test]
    fn hash_matches_derived_constants() {
        unsafe {
            assert_eq!(hash_ascii(b"kernel32.dll\0".as_ptr() as usize), KERNEL32_HASH);
            assert_eq!(hash_ascii(b"ntdll.dll\0".as_ptr() as usize), NTDLL_HASH);
            assert_eq!(hash_ascii(b"LoadLibraryA\0".as_ptr() as usize), LOADLIBRARYA_HASH);
            assert_eq!(hash_ascii(b"GetProcAddress\0".as_ptr() as usize), GETPROCADDRESS_HASH);
            assert_eq!(hash_ascii(b"VirtualAlloc\0".as_ptr() as usize), VIRTUALALLOC_HASH);
            assert_eq!(
                hash_ascii(b"NtFlushInstructionCache\0".as_ptr() as usize),
                NTFLUSH_HASH
            );
        }
        assert_eq!(ror1(0x0000_0001), 0x8000_0000);
        assert_eq!(ror1(0x8000_0000), 0x4000_0000);
    }

    #[test]
    fn peb_walk_finds_system_modules() {
        unsafe {
            let peb = peb_pointer();
            assert_ne!(peb, 0, "PEB must be readable from gs:[0x60]");
            assert_ne!(module_base_by_hash(peb, KERNEL32_HASH), 0, "kernel32.dll");
            assert_ne!(module_base_by_hash(peb, NTDLL_HASH), 0, "ntdll.dll");
        }
    }

    #[test]
    fn base_scan_finds_current_image() {
        unsafe {
            let ip = rip_here();
            let base = find_image_base(ip);
            assert_ne!(base, 0, "could not find our own image base");
            assert_eq!(rd_u16_bytes(base), 0x5A4D, "base must start with MZ");
        }
    }

    #[test]
    fn exports_resolve_by_hash() {
        unsafe {
            let peb = peb_pointer();
            let k32 = module_base_by_hash(peb, KERNEL32_HASH);
            let ntdll = module_base_by_hash(peb, NTDLL_HASH);
            assert_ne!(k32, 0);
            assert_ne!(ntdll, 0);
            assert_ne!(export_by_hash(k32, LOADLIBRARYA_HASH), 0);
            assert_ne!(export_by_hash(k32, GETPROCADDRESS_HASH), 0);
            assert_ne!(export_by_hash(k32, VIRTUALALLOC_HASH), 0);
            assert_ne!(export_by_hash(ntdll, NTFLUSH_HASH), 0);
        }
    }
}